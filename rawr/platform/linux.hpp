...
namespace rawr::platform::linux
{
    ...
    namespace x64 {
        ...
        namespace metadata
        {
            constexpr syscall_metadata write = {syscall_id::write, 1,  3, archs::x64};
            constexpr syscall_metadata exit  = {syscall_id::exit,  60, 1, archs::x64};
        }
    }
}
...
#define RAWR_PLATFORM_LINUX_SYSCALL_ON_2(canon, sem)                                         \
    constexpr self& on_##canon(auto&& fn) noexcept { if (is_##canon()) fn(); return *this; } \
    constexpr self& on_##sem(auto&& fn)   noexcept { if (is_##sem())   fn(); return *this; }
#define RAWR_PLATFORM_LINUX_SYSCALL_ON(epair) RAWR_PP_DISPATCH_PLIST_BY_ARITY(RAWR_PLATFORM_LINUX_SYSCALL_ON_, epair)

namespace rawr::platform::linux::x64::syscall
{
    ...
    RAWR_PLATFORM_LINUX_SYSCALL_RETURN(write, reg_t, rax, u64, (
    ...
    RAWR_ALWAYS_INLINE auto write(fd_t fd, char const* data, uint auto size) -> write_r{
        ...
        RAWR_PLATFORM_LINUX_X64_SYSCALL_GATED_BODY(
            ...
            if constexpr (sizeof(size) <= 4)
            {
                asm volatile("syscall"
                    : "+a"(rax)
                    : "D"(fd), "S"(data), "d"(static_cast<u32>(size))
                    : "rcx", "r11", "memory"
                );
            }
            else
            {
                asm volatile("syscall"
                    : "+a"(rax)
                    : "D"(fd), "S"(data), "d"(static_cast<u64>(size))
                    : "rcx", "r11", "memory"
                );
            }
        )
    ...
    template <aint auto Size, compilers C = this_compiler, archs A = this_arch>
    RAWR_ALWAYS_INLINE auto write(fd_t fd, char const(&data)[Size]) -> write_r {
        reg_t rax = metadata::write.number;
        RAWR_PLATFORM_LINUX_X64_SYSCALL_GATED_BODY(
            ...
            if constexpr (sizeof(Size) <= 4)
            {
                asm volatile("syscall"
                    : "+a"(rax)
                    : "D"(fd), "S"(data), "d"(static_cast<u32>(Size)), "m"(data) // Clobber only the data, not the whole memory.
                    : "rcx", "r11"
                );
            }
            else
            {
                asm volatile("syscall"
                    : "+a"(rax)
                    : "D"(fd), "S"(data), "d"(static_cast<u64>(Size)), "m"(data)
                    : "rcx", "r11"
                );
            }
        )
    ...
    template <compilers C = this_compiler, archs A = this_arch>
    RAWR_ALWAYS_INLINE auto write(fd_t fd, char data) -> write_r {
        const char arr[] = { data };
        return write<1>(fd, arr);
    }

    template <compilers C = this_compiler, archs A = this_arch>
    RAWR_ALWAYS_INLINE RAWR_NORETURN auto exit(u8 code) -> void
    {
        RAWR_PLATFORM_LINUX_X64_SYSCALL_GATED_BODY(
